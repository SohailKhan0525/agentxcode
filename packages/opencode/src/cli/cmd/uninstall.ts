...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall agentx and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AgentX Code")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @agent-qofeno/agentx-cli",
      pnpm: "pnpm uninstall -g @agent-qofeno/agentx-cli",
      bun: "bun remove -g @agent-qofeno/agentx-cli",
      yarn: "yarn global remove @agent-qofeno/agentx-cli",
      brew: "brew uninstall agentx",
      choco: "choco uninstall agentx",
      scoop: "scoop uninstall agentx",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "agentx", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".agentxcode") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AgentX Code!")
}
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# agentx" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(".agentxcode/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") &&
        (trimmed.includes(".agentxcode") || trimmed.includes(".opencode")))
    ) {
      continue
    }
