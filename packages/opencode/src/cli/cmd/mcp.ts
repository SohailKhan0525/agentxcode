...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .agentx/ subdirectory too)
  const candidates = [
    path.join(baseDir, "agentx.json"),
    path.join(baseDir, "agentx.jsonc"),
  ...
  // Default to agentx.json if none exist
  return candidates[0]
}
