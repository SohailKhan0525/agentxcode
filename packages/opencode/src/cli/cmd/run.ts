...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run agentx with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running agentx server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AGENTX_SERVER_PASSWORD or OPENCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AGENTX_SERVER_USERNAME or 'agentx')",
      })
      .option("dir", {
        type: "string",
