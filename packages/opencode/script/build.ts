...
for (const item of targets) {
  ...
  const binName = item.os === "win32" ? "agentx.exe" : "agentx"
  const legacyBinName = item.os === "win32" ? "opencode.exe" : "opencode"
  ...
  // Provide opencode alias binary for backwards compatibility
  const binPath = `dist/${name}/bin/${binName}`
  const legacyBinPath = `dist/${name}/bin/${legacyBinName}`
  await Bun.file(legacyBinPath).write(await Bun.file(binPath).arrayBuffer())
  if (item.os !== "win32") {
    await $`chmod 755 ${binPath} ${legacyBinPath}`.nothrow()
  }

  // Smoke test: only run if binary is for current platform
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    console.log(`Running smoke test: ${binPath} --version`)
    try {
      const versionOutput = await $`${binPath} --version`.text()
...
if (Script.release) {
  ...
  for (const file of new Bun.Glob("*.{zip,tar.gz}").scanSync({ cwd: "dist" })) {
