...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["agentx.json", "agentx.jsonc", "opencode.json", "opencode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".agentxcode", ".opencode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".agentxcode" || path.basename(item) === ".opencode")
        .toReversed()
