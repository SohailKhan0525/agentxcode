...
const useColor =
  process.stdout.isTTY && !process.env.NO_COLOR && process.env.TERM !== "dumb" && process.env.FORCE_COLOR !== "0"

// Detect CI environments
const isCI = Boolean(
  process.env.CI || process.env.CONTINUOUS_INTEGRATION || process.env.GITHUB_ACTIONS || process.env.GITLAB_CI,
)
